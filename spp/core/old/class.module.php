   ...
   //     $this->_setprops=array('PublicName','PublicDesc','InternalName','Version','InstallScript','UninstallScript','ModuleGroup');
    ...
    private function readYaml($file)
    {
        ...
        if (file_exists(self::$yaml_file)) {
            $mods_yaml = Yaml::parseFile(self::$yaml_file);
            $arr=$mods_yaml['modules'];
            foreach($arr as $mod)
            {
...
//            var_dump($arr);
        ...
        else
        {
            ...
            $yaml_file = SPP_APP_DIR . SPP_DS . 'etc' . SPP_DS . 'settings' . SPP_DS . 'modules' .  SPP_DS . $modname .  SPP_DS . 'config.yml';
            //echo $yaml_file;
            if (file_exists($yaml_file)) {
                $mods_yaml = Yaml::parseFile($yaml_file);
                return \SPP\Settings::getSetting($varname, 'variables', 'config.yml', $yaml_dir);
            }
...
//            return false;
    ...
    public static function getConfFile($modname,$filename)
    {
        ...
        $files[]=$file;
        $files[]=SPP_APP_DIR.'/etc/modules/'.$modname.'/config.yml';
        return $files;
    }
    ...
    public static function loadAllModules()
    {
        ...
        $mods_xml='';
        $mods_yaml='';
        $yaml_file = SPP_APP_DIR . SPP_DS . 'etc' . SPP_DS .'settings' . SPP_DS . 'modules' .  SPP_DS . 'modules.yml';
        //echo $yaml_file;
        if(file_exists($yaml_file))
        {
            $mods_yaml=Yaml::parseFile($yaml_file);
        ...
        $mods= $mods_yaml=='' ?  $mods_xml->xpath('/modules/module[status=\'active\']'):$mods_yaml['modules'];
        foreach($mods as $mod)
        {
            //var_dump($mod);
