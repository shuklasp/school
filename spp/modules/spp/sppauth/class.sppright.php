 ...
 */
class SPP_Right extends SPP_Object {
    ...
    {
        $db=new SPP_DB();
        $sql='select * from '.SPP_Base::sppTable('rights').' where rightname=?';
        $values=array($rt);
    ...
    {
        $db=new SPP_DB();
        $sql='insert into '.SPP_Base::sppTable('rights').'(rightid,rightname) values(?,?)';
    ...
    {
        $db=new SPP_DB();
        $sql='delete from '.SPP_Base::sppTable('rights').' where rightname=?';
        $values=array($rt);
        $db->execute_query($sql, $values);
    }
    ...
    {
        $db=new SPP_DB();
        $sql='select rightid from '.SPP_Base::sppTable('rights').' where rightname=?';
        $values=array($rt);
        $result=$db->execute_query($sql, $values);
        if(sizeof($result)>0)
        {
