 ...
 */
class SPP_Profile extends SPP_Object{
    ...
    public function  __construct($pname) {
        $db=new SPP_DB();
        $sql='select * from '.SPP_Base::sppTable('profiletabs').' where profname=?';
        $values=array($pname);
    ...
    function getFields()
    {
        ...
        if($this->proftabname==null||$this->proftabname=='')
                throw(new SPP_Exception ('No profile tab!'));
        $sql='show columns from '.SPP_Base::sppTable($this->proftabname);
    ...
    function seekValue($fld,$val)
    {
        ...
        if($this->proftabname==null||$this->proftabname=='')
                throw(new SPP_Exception ('No profile tab!'));
        $sql='select * from '.SPP_Base::sppTable($this->proftabname).' where '.$fld.'=?';
        $values=array($val);
        $result=$db->execute_query($sql, $values);
        if(sizeof($result)>0)
        {
    ...
    public function getAll() {
        $db=new SPP_DB();
        $sql=$sql='select * from '.SPP_Base::sppTable($this->proftabname);
        $result=$db->execute_query($sql);
        return $result;
    }
    ...
    {
        $db=new SPP_DB();
        $sql='select * from '.SPP_Base::sppTable($this->proftabname).' where '.$this->idfield.'=?';
        $values=array($pid);
        $result=$db->execute_query($sql, $values);
        if(sizeof($result)>0)
        {
        ...
        else
        {
            foreach($flds as $fld=>$val)
            {
                if(!is_int($fld))
                {
                    ...
                    {
                        throw new SPP_Exception('Invalid field '.$fld);
                    }
                }
                else
                {
    ...
    //$db->setAttribute( PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION );
        ...
        {
            $sql='update '.SPP_Base::sppTable($this->proftabname).' set ';
            $values=array();
            $i=0;
    ...
    public function appendNew()
    {
        ...
        $sql='insert into '.SPP_Base::sppTable($this->proftabname).'('.$this->idfield.') values(?)';
    ...
    //$db->setAttribute( PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION );
        ...
        try{
        $nprofid=SPP_Sequence::next($this->profseqname);
        $sql='insert into '.SPP_Base::sppTable($this->proftabname).' set ';
        $values=array();
        $i=0;
        foreach($flds as $fld=>$val)
        {
    ...
    public function deleteMe()
    {
        ...
        {
            $sql='delete from '.SPP_Base::sppTable($this->proftabname).' where '.$this->idfield.'=?';
    ...
    public static function dropProfile($pname)
    {
        ...
        if(self::doesProfileExist($pname))
        {
            ...
            //echo $tabname;
            $sql='drop table '.SPP_Base::sppTable($tabname);
            $result=$db->execute_query($sql);
            $sql='delete from '.SPP_Base::sppTable('profiletabs').' where profname=?';
            $result=$db->execute_query($sql, $values);
    ...
    public static function createProfile($pname,$flds=array())
    {
        ...
        {
            $sql='create table '.SPP_Base::sppTable('prof_'.$pname).' (profid  bigint primary key';
            ...
            $result=$db->execute_query($sql);
            $sql='insert into '.SPP_Base::sppTable('profiletabs').' values(?,?,?,?)';
